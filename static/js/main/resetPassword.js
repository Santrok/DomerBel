...
const resetPasswordBtn = document.querySelector(".modals__login-action")
  .children[0]
...
function requestResetPassword() {
  ...
  fetch(
    `${window.location.protocol}//${window.location.host}/api/v1/password_reset/`,
    {
      method: "POST",
      headers: {
    ...
    .then((data) => {
      ...
      if (data.success) {
        ...
        document.querySelector(".modals__notification-text").innerHTML =
          data.success
    ...
    .catch((error) => {
      ...
      if (errorMessage["recaptcha"]) {
        resetSubmitPasswordBtn.removeEventListener(
          "click",
